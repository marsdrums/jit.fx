...
slab.file = "jit.fx.filter.convolve.jxs";
slab.inputs = 1;

var kernelTex = new JitterObject("jit_gl_texture", drawto);
kernelTex.rectangle = 1;
kernelTex.adapt = 1;
...
function blur_amount(){ 
	...
	slab.param("blur_amount", _blur_amount);
}

function jit_matrix(inname){

	if(inlet == 1){
		kernelTex.jit_matrix(inname);
		slab.activeinput = 1;
		slab.jit_gl_texture(kernelTex.name);
	}
}
